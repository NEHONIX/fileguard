...
const testData = Buffer.from(
  "This is test data for RSA layer encryption",
  "utf8"
);
...
async function testRSALayerIsolation() {
  try {
    ...
    console.log(
      "Encrypted data extracted, size:",
      readEncryptedData.length,
      "bytes"
    );
    ...
    console.log(
      "- Keys match:",
      aesKey.equals(decryptedAesKey) ? "YES ✓" : "NO ✗"
    );
    ...
    const decrypted = Buffer.concat([
      decipher.update(readEncryptedData),
      decipher.final(),
    ]);
    ...
    console.log(
      "- Data matches:",
      testData.equals(decrypted) ? "YES ✓" : "NO ✗"
    );
