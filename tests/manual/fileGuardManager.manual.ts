...
async function runTest() {
  ...
  try {
    ...
    if (encryptResult.compressionRatio !== undefined) {
      console.log(
        `Compression ratio: ${encryptResult.compressionRatio.toFixed(2)}`
      );
    }
    ...
    console.log(
      `\nFile exists at ${binaryFilePath}, size: ${
        fs.statSync(binaryFilePath).size
      } bytes`
    );
    ...
    console.log(
      `Binary data integrity check: ${binaryIsEqual ? "PASSED ✓" : "FAILED ✗"}`
    );
    ...
    console.log(
      `Original size: ${simpleBinaryEncryptResult.size.original} bytes`
    );
    console.log(
      `Encrypted size: ${simpleBinaryEncryptResult.size.encrypted} bytes`
    );
    ...
    console.log(
      `\nFile exists at ${simpleBinaryFilePath}, size: ${
        fs.statSync(simpleBinaryFilePath).size
      } bytes`
    );
    ...
    // Verify data integrity
    const simpleBinaryIsEqual = verifyDataIntegrity(
      testData,
      simpleBinaryDecryptedData
    );
    console.log(
      `Simple binary data integrity check: ${
        simpleBinaryIsEqual ? "PASSED ✓" : "FAILED ✗"
      }`
    );
    ...
    for (const level of securityLevels) {
      ...
      // Verify data integrity
      const securityIsEqual = verifyDataIntegrity(
        testData,
        securityDecryptedData
      );
      console.log(
        `${level} security data integrity check: ${
          securityIsEqual ? "PASSED ✓" : "FAILED ✗"
        }`
      );
    }
