...
async function runTest() {
  ...
  try {
    ...
    try {
      await fgm.loadWithAdvancedDecryption(filePath, encryptionKey, rsaKeyPair, {
        disableFallbackMode: true, // Disable fallback to ensure we get the error
      });
      
      console.log('SECURITY ISSUE: Decryption of tampered file succeeded when it should have failed!');
      return false;
    } catch (error) {
    ...
    try {
      await fgm.loadWithAdvancedDecryption(keySecurityFilePath, wrongKey, rsaKeyPair, {
        disableFallbackMode: true, // Disable fallback to ensure we get the error
      });
      
      console.log('SECURITY ISSUE: Decryption with wrong key succeeded when it should have failed!');
      return false;
    } catch (error) {
    ...
    try {
      await fgm.loadWithAdvancedDecryption(keySecurityFilePath, encryptionKey, wrongRSAKeyPair, {
        disableFallbackMode: true, // Disable fallback to ensure we get the error
      });
      
      console.log('SECURITY ISSUE: Decryption with wrong RSA key pair succeeded when it should have failed!');
      return false;
    } catch (error) {
    ...
    const keysMatch = 
      newPersistentFgm.rsaKeyPair.publicKey === persistentFgm.rsaKeyPair.publicKey &&
      newPersistentFgm.rsaKeyPair.privateKey === persistentFgm.rsaKeyPair.privateKey;
